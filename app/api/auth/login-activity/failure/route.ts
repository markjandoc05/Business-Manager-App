...
import { recordMemberLoginFailure } from '@/lib/server/login-activity';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const recorded = await recordMemberLoginFailure(orgId, authenticatedUser.uid, 'WORKSPACE_ACCESS_FAILED');
    return recorded ? NextResponse.json({ ok: true }) : errorResponse('Workspace membership was not found.', 403);
  } catch (error) {
    const message = error instanceof Error ? error.message : '';
    if (message.startsWith('Invalid ')) return errorResponse('Invalid login activity request.', 400);
    console.warn('[login-activity] failure recording failed:', error);

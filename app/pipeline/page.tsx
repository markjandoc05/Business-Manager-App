...
import type { Client, Deal, DealLineItem, Task } from '@/types';
...
export default function PipelinePage() {
  ...
  const handleCreateDeal = async (event: React.FormEvent) => {
    ...
    try { await addDeal(dealForm); setDealForm({ title: '', clientId: '', value: 5000, items: [], stage: defaultDealStage, expectedCloseDate: '', lossReason: '', ...defaultAssignment }); setShowAddModal(false); } catch (createError) { console.error('Unable to create deal', createError); setError('Unable to create the deal. Please try again.'); } finally { setSaving(false); }
  };
  ...
  const filteredDeals = deals.filter((deal) => {
    ...
    const matchesSearch = deal.title.toLowerCase().includes(search) || (client?.name || '').toLowerCase().includes(search) || productServiceName.toLowerCase().includes(search);
  ...
  return <div className="space-y-4">
    {activeDealCount !== null && activeDealCount > 100 && <p className="text-xs text-[var(--app-muted)]">Showing the latest 100 active deals.</p>}
...
function StageColumn({ stage, deals, totalValue, probability, clients, tasks, busyDealId, dragOverStage, canMoveDeal, onOpen, onArchive, currency }: { stage: typeof PIPELINE_STAGES[number]; deals: Deal[]; totalValue: number; probability: number; clients: Client[]; tasks: Task[]; busyDealId: string | null; dragOverStage: string | null; canMoveDeal: (deal: Deal) => boolean; onOpen: (dealId: string) => void; onArchive: (deal: Deal) => void; currency: string }) {
  ...
  return <div ref={setNodeRef} className={`flex min-w-0 flex-col overflow-hidden rounded-lg border ${highlighted ? 'border-[var(--app-primary)] bg-[var(--app-accent-soft)]/30' : 'border-[var(--app-border)] bg-[var(--app-surface-subtle)]/70'}`} style={{ boxShadow: `inset 0 3px 0 ${stageColor}` }}><div className="sticky top-0 z-10 border-b border-[var(--app-border)] bg-white px-3 py-2.5"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold text-[var(--app-text)]">{stage}</h3><span className="text-xs font-medium text-[var(--app-muted)]">{deals.length}</span></div><div className="mt-1 flex items-center justify-between text-xs text-[var(--app-muted)]"><span>{formatCurrency(totalValue, currency)}</span><span>{probability}% probability</span></div></div><div className="min-h-[220px] max-h-[calc(100vh-280px)] flex-1 space-y-2 overflow-y-auto p-2">{deals.length === 0 ? <p className="py-8 text-center text-xs text-[var(--app-tertiary)]">No deals in this stage</p> : deals.map((deal) => <DraggableDealCard key={deal.id} deal={deal} client={clients.find((client) => client.id === deal.clientId)} tasks={tasks} canManage={canMoveDeal(deal)} busy={busyDealId === deal.id} canDrag={canMoveDeal(deal)} onOpen={() => onOpen(deal.id)} onArchive={() => onArchive(deal)} currency={currency} />)}</div></div>;
}

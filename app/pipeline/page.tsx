...
export default function PipelinePage() {
  const { clients, clientsLoading, deals, dealsLoading, tasks, settings, users, updateDealStage, updateDeal, archiveDeal, archivedDeals, loadArchivedRecords, restoreDeal, permanentlyDeleteDeal, addDeal, addTask, completeTask } = useApp();
  const { user } = useAuth();
...
function SortableDealCard({ deal, client, tasks, canManage, busy, onOpen, onArchive, currency }: { deal: Deal; client?: Client; tasks: Task[]; canManage: boolean; busy: boolean; onOpen: () => void; onArchive: () => void; currency: string }) {
  ...
  return <div ref={setNodeRef} style={style} {...attributes} {...listeners} role="button" tabIndex={0} aria-label={`Open deal ${deal.title}`} onPointerDown={() => { pointerMoved.current = false; }} onPointerMove={() => { pointerMoved.current = true; }} onClick={openFromCard} onKeyDown={handleCardKeyDown}><Card className="cursor-pointer space-y-2 rounded-md p-3 shadow-none transition-colors hover:border-slate-300 hover:bg-slate-50 focus-within:ring-2 focus-within:ring-blue-500"><div className="flex items-start justify-between gap-2"><button type="button" onClick={(event) => { event.stopPropagation(); onOpen(); }} className="line-clamp-2 text-left text-sm font-medium text-slate-900 hover:text-blue-600">{deal.title}</button>{canManage && <button type="button" title="Archive deal" disabled={busy} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); onArchive(); }} className="text-slate-400 hover:text-red-600"><Archive size={14} /></button>}</div><p className="truncate text-xs text-slate-500">{client?.name || 'Client'}</p>{deal.productServiceName && <p className="truncate text-[11px] text-slate-400">{deal.productServiceName}</p>}<div className="flex items-center justify-between gap-2 text-xs"><span className="font-semibold text-slate-800">{formatCurrency(deal.value, currency)}</span>{deal.expectedCloseDate && <span className="truncate text-slate-400">{new Date(deal.expectedCloseDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>}</div>{taskLabel && <Badge variant={overdueCount > 0 ? 'red' : 'gray'}>{taskLabel}</Badge>}</Card></div>;
}

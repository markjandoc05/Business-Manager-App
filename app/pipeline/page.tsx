...
type DealForm = { title: string; clientId: string; value: number; stage: string; expectedCloseDate: string; assignedTo: string };

export default function PipelinePage() {
  const { deals, dealsLoading, settings, updateDealStage, archiveDeal, addDeal } = useApp();
  const { user } = useAuth();
  ...
  const [dealForm, setDealForm] = useState<DealForm>({ title: '', clientId: '', value: 5000, stage: 'Opportunity', expectedCloseDate: '', assignedTo: '' });
  ...
  const loadClients = async () => {
    ...
    try { setClients(await listClients(user)); }
    catch (loadError) { console.error('Unable to load clients for pipeline', loadError); setError('Unable to load clients for deal creation.'); }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.error('Unable to load clients for pipeline', loadError);
        if (!cancelled) setError('Unable to load clients for deal creation.');
      } finally {
  ...
  const handleCreateDeal = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!canManage || !dealForm.title || !dealForm.clientId) return;
    setSaving(true); setError(null);
    try {
      await addDeal(dealForm);
      setDealForm({ title: '', clientId: '', value: 5000, stage: 'Opportunity', expectedCloseDate: '', assignedTo: '' });

...
export default function PipelinePage() {
  ...
  const changeStage = async (deal: Deal, stage: string, status: DealStatus = 'Active', lossReason?: string) => {
    if ((!canManage && !(user?.active === true && deal.assignedToUid === user.uid)) || busyDealId) return;
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!deal || (!canManage && !(user?.active === true && deal.assignedToUid === user.uid)) || deal.stage === newStage) return;

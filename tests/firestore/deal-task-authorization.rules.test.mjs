...
async function seed() {
  await testEnv.withSecurityRulesDisabled(async (context) => {
    ...
    await Promise.all([
      ...
      setDoc(doc(db, `users/${MANAGER_UID}`), { uid: MANAGER_UID, status: 'active', active: true }),
      setDoc(doc(db, `users/${USER_A_UID}`), { uid: USER_A_UID, status: 'active', active: true }),
      setDoc(doc(db, `users/${USER_B_UID}`), { uid: USER_B_UID, status: 'active', active: true }),

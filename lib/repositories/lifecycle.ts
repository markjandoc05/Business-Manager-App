...
export async function getLifecycleDecision(
  ...
  const response = await fetch(`/api/organizations/${encodeURIComponent(organizationId)}/records/${entity.toLowerCase()}/${encodeURIComponent(recordId)}?action=${encodeURIComponent(action)}`, {
...
export async function permanentlyDeleteRecord(user: AppUser | null, organizationId: string, entity: LifecycleEntity, recordId: string) {
  ...
  const token = await getFirebaseToken();
  const response = await fetch(`/api/organizations/${encodeURIComponent(organizationId)}/records/${entity.toLowerCase()}/${encodeURIComponent(recordId)}`, {
    method: 'DELETE',
...
async function requestBulkLifecycle(user: AppUser | null, organizationId: string, entity: LifecycleEntity, action: BulkLifecycleAction, recordIds: string[], mode: 'preview' | 'execute') {
  ...
  const token = await getFirebaseToken();
  const response = await fetch(`/api/organizations/${encodeURIComponent(organizationId)}/records/bulk`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
